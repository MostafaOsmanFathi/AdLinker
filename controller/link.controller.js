...
let getAllPublisherLinks = async (req, res) => {
    try {
        const result = await linkModel.find({publisher_email: req.auth_user_data.email})
        res.status(200).json(result)
    } catch (err) {
        console.log(err)
        res.status(500).send({})
...
let createLink = async (req, res) => {
    try {
        ...
        let newLinkModel = new linkModel({original_link, shorten_link, publisher_name, publisher_email})
